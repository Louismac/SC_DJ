Mixer_refactored {
	...
	*initSynthDefs {
		...
		SynthDef(\eqMixer4, { arg outBus, inBus, hiVal=0.5, midVal=0.5, loVal=0.5;
			var input, output, lo, mid, hi, loAmp, midAmp, hiAmp;
			var lpfCutoffFreq = midVal.linexp(0, 0.5, 20, 10000);
			var hpfCutoffFreq = midVal.linexp(0.5, 1, 20, 10000);
			input = InFeedback.ar(inBus, 2);
			lo = LPF.ar(input, 300);
			mid = BPF.ar(input, 1500, 2);
			hi = HPF.ar(input, 4000);
