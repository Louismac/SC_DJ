Track {
	...
	*initSynthDefs {
		SynthDef(\playTrack4, { arg bufnum=0, amp=1, length=1, panPot=0, rate=1, t_trig, startPos=0, loop=0, bus=nil, deck=0;
			var env, sound, ampTrig, playback, ampMeasure;
			env = EnvGen.ar(Env([1,1], [(BufFrames.kr(bufnum)/44100)*length]), levelScale:amp);
			ampTrig = Impulse.kr(10);

			// Play buffer (works with both mono and stereo - outputs stereo array)
			playback = PlayBuf.ar(2, bufnum, BufRateScale.kr(bufnum) * rate, t_trig, startPos, loop);
			sound = Pan2.ar(playback, panPot) * amp;

			// Amplitude measurement (read stereo and sum to mono)
			ampMeasure = PlayBuf.ar(2, bufnum, BufRateScale.kr(bufnum) * rate, t_trig, startPos, loop);
			SendReply.kr(ampTrig, 'amp', [
				Amplitude.kr(Mix.ar(ampMeasure) * amp),
				deck
			]);
		...
		SynthDef(\playTrack2, { arg bufnum=0, amp=1, length=1, rate=1, t_trig, startPos=0, loop=0, bus=nil, deck=0;
			...
			// Play buffer (works with both mono and stereo)
			playback = PlayBuf.ar(2, bufnum, BufRateScale.kr(bufnum) * rate, t_trig, startPos, loop);
			sound = playback * amp;
