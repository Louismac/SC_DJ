...
*new {arg modules;
^super.new.initPortaGUI(modules);	
}	

initPortaGUI {arg modules;
	s=Server.default;
	mixer=modules[0];
	decks=modules[1];
	midi=modules[2];
	...
	.states_([["Map",Color.yellow,Color.black],["Close",Color.black,Color.yellow]])
	.action_({
		if(midi.isLearning,{
			midi.closeGUI;
		},{
	...
	{2.do{arg i;
		var lbls=List.new(0);
		dragBox.add(TextView.new(window,Rect(75+(260*i), 55, 100,25))
		.string_(""));
	    ...
	    lbls.add(StaticText.new(window,Rect(40+(380*i),220,50,20)).string_("Cut Rate").stringColor_(Color.yellow));
	    lbls.add(StaticText.new(window,Rect(40+(380*i),180,50,20)).string_("Trig Rate").stringColor_(Color.yellow));
	    lbls.add(SCStaticText.new(window,Rect(40+(380*i),140,50,20)).string_("Cue Pos").stringColor_(Color.yellow));
		labels.add(lbls);
	}}.fork(AppClock);
}
